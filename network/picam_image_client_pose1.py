...
#picam2.configure(picam2.create_preview_configuration(main={"format": 'XRGB8888', "size": (320,180)}))
picam2.start()
...
while True:
    ...
    output1 = []
    #while True:
    output2 = client_socket.recv(1048576)
    if not output2: break
    output1.append(output2)
    
    output = pickle.loads(b"".join(output1))
    print(output)
    """
    ...
    if not output:
        break
    rec_output = str(output.decode(encoding='utf-8'))
    if rec_output.__contains__('|||'):
        if not rec_output.__contains__('|/|'):
            rec_output = rec_output.replace('|||','')
            output_tmp = output_tmp + rec_output
            continue
        else:
            rec_output = rec_output.replace('|||','')
            rec_output = rec_output.replace('|/|','')
            output_tmp = output_tmp + rec_output
            rec_output = output_tmp
    ...
    """
    output1 = client_socket.recv(1048576)
    output1 = output1.decode('utf-8')
    output2 = json.loads(output1)
    output = np.asarray(output2)
    """
    #output3 = output2.decode()
    
    #output = np.array(output3)
    ...
    #print(frame1.ndim)
    #print(type(frame1))
    # output : 2 dim , numpy.ndarray
    # frame1 : 3 dim, numpy.ndarray
    ...
    show_message(device, "%d"%output.shape[0], fill="white", font=proportional(CP437_FONT))
